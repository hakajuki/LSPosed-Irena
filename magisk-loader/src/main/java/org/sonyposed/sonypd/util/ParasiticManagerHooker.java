...
import static org.sonyposed.sonypd.core.ApplicationServiceClient.serviceClient;
...
public class ParasiticManagerHooker {
    ...
    private static void sendBinderToManager(final ClassLoader classLoader, IBinder binder) {
        try {
            var clazz = XposedHelpers.findClass("org.sonyposed.manager.Constants", classLoader);
                ...
                for (var i = 0; i < param.args.length; ++i) {
                    if (param.args[i] instanceof ActivityInfo) {
                        ...
                        for (var activity : pkgInfo.activities) {
                            if ("org.sonyposed.manager.ui.activity.MainActivity".equals(activity.name)) {
                    ...
                    if (param.args[i] instanceof Intent) {
                        ...
                        intent.setComponent(new ComponentName(intent.getComponent().getPackageName(), "org.sonyposed.manager.ui.activity.MainActivity"));
                    }
                }
