...
import static org.sonyposed.sonypd.service.PackageService.MATCH_ALL_FLAGS;
import static org.sonyposed.sonypd.service.PackageService.PER_USER_RANGE;
import static org.sonyposed.sonypd.service.ServiceManager.TAG;
import static org.sonyposed.sonypd.service.ServiceManager.existsInGlobalNamespace;
import static org.sonyposed.sonypd.service.ServiceManager.toGlobalNamespace;
...
import org.sonyposed.daemon.BuildConfig;
import org.sonyposed.sonypd.models.Application;
...
public class ConfigManager {
    ...
    private synchronized void updateConfig() {
        Map<String, Object> config = getModulePrefs("sonypd", 0, "config");
        ...
        if (string == null) {
            ...
            updateModulePrefs("sonypd", 0, "config", "misc_path", miscPath.toString());
        } else {
            ...
            switch (db.getVersion()) {
                case 0:
                    executeInTransaction(() -> {
                        ...
                        values.put("module_pkg_name", "sonypd");
    ...
    private synchronized void cacheModules() {
        ...
        try (Cursor cursor = db.query(true, "modules", new String[]{"module_pkg_name", "apk_path"},
            ...
            while (cursor.moveToNext()) {
                ...
                if (packageName.equals("sonypd")) continue;
                var module = new Module();
    ...
    public List<Application> getModuleScope(String packageName) {
        if (packageName.equals("sonypd")) return null;
    ...
    public boolean updateModuleApkPath(String packageName, String apkPath, boolean force) {
        if (apkPath == null || packageName.equals("sonypd")) return false;
        if (db.inTransaction()) {
    ...
    private int getModuleId(String packageName) {
        if (packageName.equals("sonypd")) return -1;
        if (db.inTransaction()) {
    ...
    public String[] enabledModules() {
        try (Cursor cursor = db.query("modules", new String[]{"module_pkg_name"}, "enabled = 1", null, null, null, null)) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (pkgName.equals("sonypd")) continue;
                result.add(pkgName);
            }
    ...
    private boolean removeModuleWithoutCache(String packageName) {
        if (packageName.equals("sonypd")) return false;
    ...
    private boolean removeModuleScopeWithoutCache(Application module) {
        if (module.packageName.equals("sonypd")) return false;
    ...
    public void setVerboseLog(boolean on) {
        ...
        updateModulePrefs("sonypd", 0, "config", "enable_verbose_log", on);
        verboseLog = on;
    }
    ...
    public void setDexObfuscate(boolean on) {
        updateModulePrefs("sonypd", 0, "config", "enable_dex_obfuscate", on);
    }
    ...
    public void blockScopeRequest(String packageName) {
        ...
        set.add(packageName);
        updateModulePrefs("sonypd", 0, "config", "scope_request_blocked", set);
    ...
    boolean dexObfuscate() {
        var bool = getModulePrefs("sonypd", 0, "config").get("enable_dex_obfuscate");
    ...
    public void setEnableStatusNotification(boolean enable) {
        updateModulePrefs("sonypd", 0, "config", "enable_status_notification", enable);
